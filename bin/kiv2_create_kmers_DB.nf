...
process extract_fasta_from_bed {
    ...
    script:
        ...
        awk '{print \$1":"\$2"-"\$3}' ${region_bed} > region.regions
        ${params.tools.samtools} faidx ${params.input.genome_fasta} -r region.regions > ${region_fasta}
        """
}
...
process count_kmers_outside_region {
    ...
    script:
        ...
        awk '{ print \$1"\\t1\\t"\$2 }' ${params.input.genome_fai} > genome.bed;
        
        ${params.tools.bedtools} intersect -v -a genome.bed -b ${region_bed} | awk '{print \$1":"\$2"-"\$3}' > genome_without_region.regions;
        ${params.tools.samtools} faidx ${params.input.genome_fasta} -r genome_without_region.regions > genome_without_region.fasta;
...
workflow prepare_kmers_kiv2 {
	main:
	    ...
	    KIV2_fasta = extract_fasta_from_bed(KIV2_bed)
        kiv2_kmers = count_kmers_region(KIV2_fasta)
	    
	    kiv2_kmers_filt = filter_on_occurence(kiv2_kmers, 6)
	    
	    kiv2_bed = Channel.fromPath("${params.input.kiv2_bed}")
	    kiv2_outside_kmers = count_kmers_outside_region(kiv2_bed)
...
workflow prepare_kmers_norm {
    main:
	    ...
	    norm_fasta = extract_fasta_from_bed(norm_bed)
        norm_kmers = count_kmers_region(norm_fasta)
	    
	    norm_kmers_filt = filter_on_occurence(norm_kmers, 1)
	    
	    norm_bed = Channel.fromPath("${params.input.norm_bed}")
	    norm_outside_kmers = count_kmers_outside_region(norm_bed)
