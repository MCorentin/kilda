...
# kmers on the KIV2 and LPA region.

# The KIV2 kmers were selected to appear accross the 6 KIV2 copies of the reference genome
# The LPA kmers were selected to appear only once on the LPA region of the reference genome
...
def usage():
    ...
    print("     -l/--lpa             File listing the curated LPA kmers. (required, default = './LPA_unique_21mer_with_rc.tsv')")
...
def check_arguments(counts_list_file, kiv_kmers_file, lpa_kmers_file, rsids_kmers_file, output_folder):
    ...
    Args:
        ...
        lpa_kmers_file: a string containing the path to the file with the curated LPA kmers.
...
def get_kmer_CN(kiv_counts, lpa_counts, verbose_on):
    """Compute the KIV2 CN based on the ratio of the mean occurence of KIV2 kmers vs LPA kmers

    Args:
        kiv_counts: A pandas DataFrame with the KIV2 kmer occurences.
        lpa_counts: A pandas DataFrame with the LPA  kmer occurences.
    ...
    if(verbose_on): print("\tLPA  mean kmer occurence:'%.2f'" % lpa_mean)

    if(kiv_mean == 0 or lpa_mean == 0):
        sys.exit("Error: the mean occurence for the KIV2 or LPA kmers is 0: cannot get the ratio.")
...
def plot_occurences(kiv_counts, lpa_counts, out_filename, sample_id, CN):
    """Plot the distribution of kmer occurences for the KIV2 and LPA kmers
...
def main():
    ...
    if(verbose_on): print("Identified '"+str(lpa_kmers_df.shape[0])+"' LPA curated kmers")
    ...
    if(not(set(kiv_kmers_df['kmer']).isdisjoint(lpa_kmers_df['kmer']))):
        print("Warning ! There is some overlap between the KIV2 kmers and the LPA kmers, is this expected?\n")   
    ...
    with open(counts_list_file, "r") as counts_handler:
        for counts_row in counts_handler:
            ...
            if(verbose_on): print("\tLPA kmers with 0 counts: "+ str(lpa_counts_df.loc[lpa_counts_df['occurence'] == 0].shape[0]) + " / " + str(lpa_counts_df.shape[0]))
            if(verbose_on): print("\tUnknown kmers: "+ str(unknown_df.shape[0]))

            if(kiv_counts_df.shape[0] == 0 or lpa_counts_df.shape[0] == 0):
                sys.exit("Error, no LPA and/or KIV2 kmers found for sample '"+ str(sample_id) +"'")
